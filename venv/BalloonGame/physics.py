    ...
    def move(self):
        ...
        if not self.is_edge():
            ...
            #撞到墙壁的特殊处理
                ...
                if self.collidect_wall == consts.RectBorder.TOP or self.collidect_wall == consts.RectBorder.BOTTOM:
                    self.position = (self.position[0] + self.speed[0], self.position[1])
                    # 移动矩形
                    self._rect[0] = self.position[0]
                    self._rect[1] = self.position[1]
                elif self.collidect_wall == consts.RectBorder.LEFT or self.collidect_wall == consts.RectBorder.RIGHT:
