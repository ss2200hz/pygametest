...
#获取矩形碰撞位置,返回第一个矩形的碰撞位置
def get_colliderect_pos(rect_1,rect_2):
    right_direct = abs(rect_1[0] + rect_1[3] - rect_2[0])
    top_direct = abs(rect_1[1] - (rect_2[1] + rect_2[3]))
    left_direct = abs(rect_1[0] - (rect_2[0] + rect_2[2]))
    bottom_direct = abs(rect_1[1] + rect_1[3] - rect_2[1])
    if right_direct <= 5:
        return consts.RectBorder.RIGHT
    if top_direct <= 5:
        return consts.RectBorder.TOP
    if left_direct <= 5:
        return consts.RectBorder.LEFT
    if bottom_direct <= 5:
        return consts.RectBorder.BOTTOM

# if __name__ == '__main__':
#     print(get_colliderect_pos((50,50,40,40),(90,51,40,40)))
